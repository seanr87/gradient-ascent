...
- **League chat is unreadable by API.** Replies happen only when Sean screenshots it. Display names in `2026-wk02-sun-chat`, plus **theBigScone = Lost in the Land of Love** (`2026-wk03-fri-chat`). Live replies are written as a `kind: chat` file the same day; Monday's slot keeps the scheduled message.
